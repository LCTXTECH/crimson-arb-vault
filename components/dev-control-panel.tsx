...
export function DevControlPanel() {
    ...
    try {
      ...
      if (response.ok) {
        setFeedbackHistory((prev) => [
          { id: Date.now().toString(), message: feedbackText, category: feedbackCategory, timestamp: new Date() },
          ...prev,
        ])
        setFeedbackText("")
      }
  ...
  const handleSimulateFundingSpike = () => {
    console.log("[v0] Simulating funding spike:", fundingRate)
  }

  const handleResetSandbox = () => {
    console.log("[v0] Resetting sandbox")
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed right-4 bottom-4 z-50 flex items-center gap-2 rounded-lg bg-warning/20 border border-warning/50 px-4 py-2 text-sm font-mono text-warning hover:bg-warning/30 transition-colors"
      >
          ...
          <div className="fixed right-0 top-0 h-full w-96 bg-[#0a0e14] border-l border-border z-50 flex flex-col overflow-hidden">
              ...
              {(["simulate", "logs", "feedback"] as const).map((tab) => (
                <button
                  ...
                  className={`flex-1 px-4 py-2 text-xs font-mono uppercase tracking-wider transition-colors ${
                    activeTab === tab ? "bg-crimson/10 text-crimson border-b-2 border-crimson" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {activeTab === "simulate" && (
                <div className="space-y-6">
                  <div className="space-y-3">
                    <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider">Simulate Funding Spike</label>
                    ...
                    <button
                      onClick={handleSimulateFundingSpike}
                  ...
                  <div className="space-y-3">
                    <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider">Simulate Liquidity Event</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button className="py-2 px-3 bg-success/10 border border-success/30 rounded text-xs font-mono text-success hover:bg-success/20 transition-colors">WHALE BUY</button>
                      <button className="py-2 px-3 bg-crimson/10 border border-crimson/30 rounded text-xs font-mono text-crimson hover:bg-crimson/20 transition-colors">WHALE SELL</button>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider">Sentry Brain State</label>
                    <div className="grid grid-cols-3 gap-2">
                      <button className="py-2 px-2 bg-muted border border-border rounded text-xs font-mono text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors">IDLE</button>
                      <button className="py-2 px-2 bg-warning/10 border border-warning/30 rounded text-xs font-mono text-warning hover:bg-warning/20 transition-colors">EVALUATING</button>
                      <button className="py-2 px-2 bg-success/10 border border-success/30 rounded text-xs font-mono text-success hover:bg-success/20 transition-colors">EXECUTING</button>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-border">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 text-center">Clears all simulated data and resets to $100k USDC</p>
                  </div>
                </div>
              )}
              ...
              {activeTab === "logs" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button className="text-xs text-crimson hover:text-crimson/80 transition-colors">Clear</button>
                  </div>
                  <div className="space-y-3">
                    {sdkLogs.map((log) => (
                      <div key={log.id} className="bg-muted/30 rounded border border-border overflow-hidden">
                        <div className="flex items-center justify-between px-3 py-2 bg-muted/50 border-b border-border">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs font-mono uppercase ${log.type === "drift" ? "text-blue-400" : log.type === "ranger" ? "text-purple-400" : "text-green-400"}`}>{log.type}</span>
                        ...
                        <pre className="p-3 text-xs font-mono text-muted-foreground overflow-x-auto">{log.payload}</pre>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === "feedback" && (
                <div className="space-y-4">
                  <div className="space-y-3">
                    <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider">Category</label>
                    <select
                  ...
                  <div className="space-y-3">
                    <label className="block text-xs font-mono text-muted-foreground uppercase tracking-wider">Feedback Message</label>
                    <textarea
                      value={feedbackText}
                      ...
                      {feedbackHistory.map((entry) => (
                        <div key={entry.id} className="bg-muted/30 rounded border border-border p-3">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-xs font-mono text-muted-foreground">{entry.timestamp.toLocaleTimeString()}</span>
                          </div>
