/**
 * Seed AI Decisions - Populates the ai_decisions table with realistic SKIP and GUARD entries
 * 
 * This script creates 20+ decisions to demonstrate the Sentry's "Discipline" to judges.
 * Run with: npx ts-node scripts/seed-ai-decisions.ts
 * Or use the API: POST /api/decisions/seed
 */
...
// Realistic market scenarios that justify SKIP/GUARD decisions
...
function generateTimestamps(count: number): Date[] {
  ...
  const interval = (24 * 60 * 60 * 1000) / count // Spread over 24h
...
export async function seedAIDecisions() {
  console.log("[v0] Starting AI Decisions seeding...")
  
  const allScenarios = [
    ...SKIP_SCENARIOS.map(s => ({ ...s, decision_type: "SKIP" })),
    ...GUARD_SCENARIOS.map(s => ({ ...s, decision_type: "GUARD" })),
    // Add a few EXECUTE decisions for contrast
    {
  ...
  console.log(`[v0] Successfully seeded ${decisions.length} AI decisions`)
  console.log(`[v0] Breakdown: ${SKIP_SCENARIOS.length} SKIP, ${GUARD_SCENARIOS.length} GUARD, 2 EXECUTE, 1 DEFER`)
